...
/// return the clients ephemeral public key.
    ...
    Sum<Sum<NonceLen, KG::PkLen>, OutputSize<D>>: ArrayLength<u8>,
{
    ...
    fn build_ke2(
        ...
        pk2_sk2_secret: Self::KE2BuilderInput,
    ) -> Result<GenerateKe2Result<Self, D, KG>, ProtocolError> {
        let result = derive_3dh_keys::<D, KG>(
            TripleDhComponents {
                pk1: builder.client_e_pk.clone(),
                sk1: builder.server_e.private().clone(),
                pk2_sk2_secret,
                pk3: builder.client_s_pk.clone(),
                sk3: builder.server_e.private().clone(),
            },
        ...
        Ok((
            ...
            Ke2Message {
                ...
                server_e_pk: builder.server_e.public().clone(),
                mac,
            },
            #[cfg(test)]
            result.3,
    ...
    fn generate_ke3<'a, 'b, 'c, 'd>(
        ...
        let result = derive_3dh_keys::<D, KG>(
            TripleDhComponents {
                ...
                pk2_sk2_secret: ke1_state.client_e_sk.ke_diffie_hellman(&server_s_pk),
...
struct TripleDhComponents<KG: KeGroup> {
    pk1: PublicKey<KG>,
    sk1: PrivateKey<KG>,
    pk2_sk2_secret: GenericArray<u8, KG::PkLen>,
    pk3: PublicKey<KG>,
    sk3: PrivateKey<KG>,
}
    ...
    Le<<D::Core as BlockSizeUser>::BlockSize, U256>: NonZero,
{
    ...
    hkdf.input_ikm(&dh.sk1.ke_diffie_hellman(&dh.pk1));
    hkdf.input_ikm(&dh.pk2_sk2_secret);
    hkdf.input_ikm(&dh.sk3.ke_diffie_hellman(&dh.pk3));
    ...
    Le<<D::Core as BlockSizeUser>::BlockSize, U256>: NonZero,
{
    fn drop(&mut self) {
        ...
        let _ = AssertZeroizeOnDrop(self.server_e.public());
        let _ = AssertZeroizeOnDrop(self.server_e.private());
        let _ = AssertZeroizeOnDrop(&self.client_s_pk);
    }
}
