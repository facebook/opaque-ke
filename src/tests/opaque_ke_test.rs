...
fn generate_parameters<CS: CipherSuite>() -> TestVectorParameters {
    ...
    let blinding_factor_bytes =
        <<CS::Group as Group>::Scalar as SizedBytes>::to_arr(&client_registration.blinding_factor)
            .clone();
    ...
    let oprf_key_bytes =
        <<CS::Group as Group>::Scalar as SizedBytes>::to_arr(&server_registration.oprf_key).clone();
